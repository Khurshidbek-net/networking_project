...
export class AnalyticsService {
  ...
  async getDashboardAnalytics() {
    const [totalInventory, pendingOrders, inboundToday, outboundToday, lowStockCount, recentActivities] =
      await Promise.all([
  ...
  async getInventoryAnalytics() {
    const [totalItems, totalValue, turnoverRate, topMovingProducts, warehouseUtilization, categoryDistribution] =
      await Promise.all([
        ...
        this.prisma.inventory
          ...
          .then((items) =>
            items.reduce((sum, item) => sum + item.quantityTotal * Number(item.product.unitPrice || 0), 0),
          ),
  ...
  async getOrderAnalytics(period = "month") {
    ...
    const [totalOrders, ordersByStatus, totalValue, averageOrderValue, fulfillmentRate, topCustomers] =
      await Promise.all([
        this.prisma.order.count({
  ...
  async getPerformanceMetrics() {
    const [inventoryTurnover, orderFulfillmentRate, warehouseUtilization, averagePickTime, onTimeDeliveryRate] =
      await Promise.all([
        this.calculateTurnoverRate(),
  ...
  private async getLowStockCount() {
    ...
    return inventory.filter((item) => item.quantityAvailable <= (item.product.minimumStock || 0)).length
  }
  ...
  private async calculateAverageWarehouseUtilization() {
    ...
    return warehouses.length > 0 ? Math.round(totalUtilization / warehouses.length) : 0
  }
